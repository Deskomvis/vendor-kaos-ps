...
export default function DtfPriceList() {
  return <section id="price-list" className="bg-white py-14 text-neutral-950 md:py-20" aria-labelledby="price-list-title">
    <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
      ...
      <details className="group rounded-2xl border border-neutral-200 bg-neutral-50 p-4"><summary className="flex cursor-pointer list-none items-center justify-between gap-4 text-sm font-black marker:hidden"><span>Lihat price list lengkap</span><ChevronDown className="h-5 w-5 transition-transform group-open:rotate-180" aria-hidden="true" /></summary><div className="mt-4 grid gap-4 border-t border-neutral-200 pt-4 sm:grid-cols-2">{priceGroups.map((group) => <article key={group.title} className="rounded-2xl border border-neutral-200 bg-white p-5"><h3 className="text-base font-black">{group.title}</h3><div className="mt-4 divide-y divide-neutral-200 rounded-xl border border-neutral-200">{group.rows.map((row) => <div key={row.label} className="flex items-center justify-between px-4 py-3 text-sm"><span className="text-neutral-600">{row.label}</span><span className="font-black">Rp {row.price}</span></div>)}</div></article>)}</div><p className="mt-5 text-xs text-neutral-500">* Harga dapat menyesuaikan desain, warna kaos, dan jumlah pesanan. Konfirmasi harga final melalui WhatsApp.</p></details>
    </div>
  </section>;
}
